...
class LangchainMCPClient:
    ...
    async def initialize_agent(self) -> None:
        ...
        try:
            mcp_tools = await self._get_and_verify_tools()
            self.tools = self._create_tools(mcp_tools)
    ...
    async def _get_and_verify_tools(self):
    ...
    def _create_tools(self, mcp_tools):
        ...
        async def read_data_wrapper(query: str = "SELECT * FROM people") -> str:
            try:
                tool = mcp_tools[1]
                if not tool:
    ...
    def _create_prompt(self):
        ...
        return ChatPromptTemplate.from_messages([system_message, human_message]).partial(
    ...
    async def process_message(self, user_input: str) -> str:
        ...
        try:
            ...
            error_msg = f"Error processing message: {type(e).__name__} - {e}\nPlease try rephrasing your request."
            logger.info("\nError processing message: %s - %s", type(e).__name__, e)
            logger.info("Full error: %s", e.__dict__)
            return error_msg
